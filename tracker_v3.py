...
def plot_track(tc,tc_forecast_list): #绘制best track和所有forecast的track
    ...
    trace = go.Scattermapbox(
        ...
        size = 5,
        color = [get_color_based_on_intensity(point[3], angle) for point, angle in zip(tc.track, np.insert(tc.angle, [0, len(tc.angle)], np.nan))],
    ),
    text =  [f"名字：{tc.name}<br>编号：{tc.number}<br>时间: {point[0]}<br>经度: {point[2]}<br>纬度: {point[1]}<br>等级：{point[3]}<br>气压：{point[4]}<br>风速：{point[5]}" + (f"<br>折角: {angle}" if i > 0 and i < tc.point - 1 else '') for i, (point, angle) in enumerate(zip(tc.track, np.insert(tc.angle, [0, len(tc.angle)], np.nan)))],
)
    ...
    for i in range(len(tc_forecast_list)):
        trace_forecast = go.Scattermapbox(
            ...
            marker=go.scattermapbox.Marker(
            size = 5,
            color = ['red' if point[4] < 5*10**-5 else 'Powderblue' for point in tc_forecast_list[i].track],
        ),
        text =  [f"名字：{tc_forecast_list[i].name}<br>编号：{tc_forecast_list[i].number}<br>时间: {point[0]}<br>经度: {point[2]}<br>纬度: {point[1]}<br>气压：{point[3]}<br>相对涡度：{point[4]}" for point in tc_forecast_list[i].track],
    )
